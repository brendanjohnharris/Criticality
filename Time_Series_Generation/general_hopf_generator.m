function [timeSeriesData, r, theta, labels, keywords] = general_hopf_generator(varargin)
% GENERAL_HOPF_GENERATOR Produces time series for hctsa from the general 
%   normal form of a Hopf bifurcation.
%
...
%
%   'lambda' is the shape parameter, determining whether the
%   hopf bifurcation that occurs is subcritical or supercritical.
%   
...
%
%   See 'A Canonical Model of Multistability and Scale-Invariance in
%   Biological Systems' (Freyer et al., 2012) for parameter details.
%
%   Note: Plotting functionality currently requires 'numSubplots' by Rob Campbell
    ...
    switch method
        case 'Euler-Maruyama'
            for n = 1:numpoints-1
                ...
                r(:, n+1) = r(:, n) + (-r(:, n).^5 + lambda.*(r(:, n).^3) + betarange'.*r(:, n)).*dt + W(:, n);
            end
        case 'Heun'
            for n = 1:numpoints - 1
                theta(:, n+1) = theta_c*(dt*n) + theta(:, 1);
                rtemp = r(:, n) + (-r(:, n).^5 + lambda.*(r(:, n).^3) + betarange'.*r(:, n)).*dt + W(:, n);
                r(:, n+1) = r(:, n) + ((-r(:, n).^5 + lambda.*(r(:, n).^3) + betarange'.*r(:, n))...
                            + (-rtemp.^5 + lambda.*(rtemp.^3) + betarange'.*rtemp))*(dt/2)...
    ...
    if p.Results.savedata
        if isempty(p.Results.foldername)
            folder = sprintf('lambda_of_%g', lambda);
        else
        ...
        save(fullfile(folder, sprintf('lambda_of_%g.mat', lambda)),...
