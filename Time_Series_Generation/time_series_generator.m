...
%
%   Input parameters (options) can be given as name-value pairs or left as 
%   defaults (as below); these should be adjusted as necessary. 
%   Note that the form and accepted type of the 'parameters' variable is
%   dependent on the specified system.
%
...
%                               TS_compute for detail on these options'
%                               purposes.
%                               Useful when the time series woudl be too
%                               large to save, or when there are too many
%                               values of eta for time series to fit in memory
...
%% Calculate which of numsteps, tmax and dt were not specified
    if ~isempty(dt) && ~isempty(tmax) && ~isempty(numpoints) % Which one to calculate is ambiguous, and they might disagree. 
