function sys = subcritical_hopf_strogatz()
    ...
    sys.pardef = [ struct('name','mu',    'value',-0.1, 'lim', [-1, 1]);
                   struct('name', 'omega', 'value', pi, 'lim', [0, 2.*pi]);
                   struct('name','eta', 'value', 0.1, 'lim', [0, 1])];
               
    % Our SDE variables
    sys.vardef =  [struct('name','x', 'value', 1, 'lim',[-1 1]);
                    struct('name','y', 'value', 1, 'lim',[-1 1])];
   ...
   % Specify SDE solvers and default options
    ...
    sys.panels.bdLatexPanel.latex = {'\textbf{Subcritical Hopf Strogatz}'};
...
function F = sdeF(~,Y,mu, omega, ~)  
    ...
    F(1, 1) = (mu + norm(Y).^2 - norm(Y).^4).*Y(1) - Y(2).*omega;
    F(2, 1) = (mu + norm(Y).^2 - norm(Y).^4).*Y(2) + Y(1).*omega;
end
