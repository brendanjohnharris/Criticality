function datastruct = light_TS_compute(doParallel,ts_id_range,op_id_range,computeWhat,customFile,beVocal, data) % M
% LIGHT_TS_COMPUTE A slightly modified version of TS_compute, for use
% with large datasets that won't be written to a file. 
% Changes from hctsa 0.98:
%     - Additional input argument 'data', as produced by light_TS_init
%     - Unnecessary lines commented out 
%     - The output 'datastruct' contains the data that would otherwise have
%       been saved to 'HCTSA.mat', excluding the bulky time series
%
% :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
% --------------------------------------------------------------------------
[TS_DataMat,TimeSeries,Operations] = TS_LoadData(data); % M
%fileVarsStruct = whos('-file',customFile);
fileVars = fieldnames(data); % M
if ~all(ismember({'TimeSeries','Operations','MasterOperations','TS_DataMat'},fileVars))
	error('\nCannot compute: missing variables.'); % M
end
MasterOperations = data.MasterOperations; % M
if ismember('TS_CalcTime',fileVars)
	TS_CalcTime = data.TS_CalcTime; % M
end
if ismember('TS_Quality',fileVars)
	TS_Quality = data.TS_Quality; % M
end
...
if any(numCalc_all > 0)
	...
	saveTimer = tic;
    fromDatabase = data.fromDatabase; % M
    gitInfo = data.gitInfo; % M
	% save(customFile,'TS_DataMat','TS_CalcTime','TS_Quality','Operations','MasterOperations','fromDatabase','gitInfo', '-v7.3') % M
    datastruct = struct('TS_DataMat', {TS_DataMat}, 'TS_CalcTime', {TS_CalcTime}, 'TS_Quality', {TS_Quality}, ... % M
                'Operations', {Operations}, 'MasterOperations', {MasterOperations}, 'fromDatabase', {fromDatabase}, 'gitInfo', {gitInfo}); % M
	fprintf(1,' Returned in %s.\n',BF_thetime(toc(saveTimer))); % M
	clear saveTimer
end
